...
import recordingSign from "../../assets/icons/recorderSignIcon.svg";
...
const RenderVoiceRecorder = ({
  ...
  const stopRecordingWithoutApiCall = async () => {
  ...
  const handleTranscription = async (blobData) => {
    ...
    try {
      console.log('audio formData-----:', formData)
      let resp = await axios.post(`/api/speechToText`, formData, {
        body: formData,
      });

      setInputMsg(resp?.data?.message?.text);
  ...
  return (
    <>
      ...
      {!showRecording ? (
        ...
        >
          <Tooltip 
          label="You can speak or type in any language, responses will be automatically converted to English." 
          placement="top"
          >
            <TooltipCard>
              <svg
        ...
        <div className={styles.recorderContainer}>
          <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
            <img
              ...
              onClick={stopRecordingWithoutApiCall}
            />
            <span
              style={{
                color: "#2B4078",
          ...
          <div style={{ display: "flex", gap: "20px", alignItems: "center" }}>
            <div className={styles.waveContainer}>
              {Array(20)
                .fill(0)
                .map((item, i) => {
                  return <div className={styles.wave} key={i}></div>;
                })}
            </div>
            <span
              className={styles.opacity}
              style={{
                backgroundColor: "#76C63B",
                padding: "17px 14px",
                borderRadius: "8px",
                cursor: "pointer",
              }}
            ...
            >
              <img src={recordingSign.src} alt={"icon"} />
            </span>
          </div>
        </div>
      )}

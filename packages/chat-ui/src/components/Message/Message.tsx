...
const Message = (props: MessageProps) => {
  ...
  console.log('content?.data?.position:', content?.data?.position)
  ...
  return (
    <div className={clsx('Message', position)} data-id={id} data-type={type}>
      ...
      <div className="Message-main">
        {isRL && content?.data?.user?.avatar ? <Avatar src={content?.data?.user?.avatar} alt={'profile'} url={user?.url} /> : <div style={{ marginLeft: content?.data?.position === 'left' ? '40px' : '' }}></div>}
